...
20-1. **UA 이름 명단으로 봇을 막는 건 두더지 잡기다 — 상대가 이름을 갈아탄다 (2026-08-19 확인).** 8/18에 robots.txt와 WAF 커스텀 규칙으로 `meta-externalagent`와 `GPTBot`을 막았다. 다음 날 확인해보니 그 둘은 **정확히 0이 됐는데**, Meta가 `meta-webindexer/1.1`이라는 **명단에 없던 새 UA로 갈아타고** 똑같이 홈만 두들기고 있었다. 하루 총 105,389 요청으로 한도를 또 넘겼다.
    ...
    **차단해도 봇은 계속 온다 — 살아있는지 보려면 두 숫자의 차이를 봐라 (2026-08-20 확인).** 대응 다음 날에도 `meta-webindexer`는 36,112회(존 도착의 97.2%) 두드리고 있었다. 달라진 건 그게 Worker까지 못 온다는 것뿐이다. 그래서 방어가 살아있는지는 **존 도착 요청 수와 Worker 실행 횟수의 차이**로 판별한다:

    | 날짜 | 존 도착 | Worker 실행 | WAF 흡수 |
    |---|---|---|---|
    | 8/17 (무방비) | 148,807 | 148,137 | 670 |
    | 8/18 (이름 갈아탄 날) | 136,816 | 104,707 | 32,109 |
    | 8/19 (3층 적용 후) | 37,104 | **878** | 36,226 |

    **두 숫자가 가까워지면 새 이름으로 갈아탄 것이다.** 요청 총량이 한도에 닿기 전에 잡히므로 Cloudflare 경고 메일(75% = 7만 5천 건을 이미 맞은 뒤)보다 훨씬 빠르다. 점검할 때 이것부터 볼 것 — 존 도착은 `httpRequests1dGroups`(zone), Worker 실행은 `workersInvocationsAdaptive`(account).

    참고로 봇 자신의 요청량도 136,816 → 37,104로 줄었다. 403을 계속 받아 속도를 낮춘 것으로 보이나 확인된 사실은 아니다.
...
- [x] ~~**"종합" 정렬이 추천수 없는 사이트(82cook)에 불리한지 검증 필요**~~ — **2026-08-18 확인됨.** 최근 24시간 기준 `cook82`와 `mlbpark`은 추천수가 중앙값·최대값 모두 **0**이라 `추천수 × 10` 가중치를 구조적으로 하나도 못 받는다(밀리는 게 맞다). 참고로 추천수가 있는 사이트에서도 기여도는 낮다 — 펨코 2%, 웃대 11% 수준이라 현재 "종합"은 사실상 조회수 정렬과 거의 같다.
...
- [ ] **목록 페이지네이션이 OFFSET 방식이라 글이 누락될 수 있다** — 2026-08-19에 "같은 글이 두 번 보인다" 제보로 드러난 문제의 나머지 절반. 정렬 점수(조회수+추천수×10)를 크롤러가 5분마다 갱신하고 시간 창(`now - N시간`)도 요청마다 새로 계산돼서, 페이지를 받는 사이에 순위가 바뀐다. 밀려난 글은 두 번 실리고(→ 프론트 dedup으로 막음, 커밋 6243366) **올라간 글은 두 페이지 어디에도 안 실려 조용히 누락된다.** 이쪽은 아직 안 고쳤다. 해결하려면 ① 첫 요청에서 절대 cutoff를 계산해 이후 페이지에 넘겨 시간 창을 고정하고 ② 커서(keyset) 페이지네이션으로 전환해야 하는데, 뒤로가기 복원 로직(index.astro)이 offset에 의존하고 있어 같이 손봐야 한다. 누락은 사용자가 알아채기 어려워 우선순위는 낮게 뒀다.
